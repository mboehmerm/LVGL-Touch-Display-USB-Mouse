...
#define USE_LGFX_FILE   // Comment this line, to use the configuratio below
#ifdef USE_LGFX_FILE
...
//  cb_queue  = xQueueCreate( 10, sizeof( struct cbMessage) );
//  key_queue = xQueueCreate( 10, sizeof( uint8_t ) );           //  typedef uint8_t lv_key_t;
...
{
  //lv_obj_t * win2 = lv_win_create(lv_scr_act(), 40);
  win2 = lv_win_create(lv_scr_act(), 40);
  lv_obj_t * btn;
  btn = lv_win_add_btn(win2, LV_SYMBOL_LEFT, 40);
  lv_obj_add_event_cb(btn, event_handler, LV_EVENT_CLICKED, NULL);

  lv_win_add_title(win2, "A title");

  btn = lv_win_add_btn(win2, LV_SYMBOL_RIGHT, 40);
  lv_obj_add_event_cb(btn, event_handler, LV_EVENT_CLICKED, NULL);

  btn = lv_win_add_btn(win2, LV_SYMBOL_CLOSE, 60);
  lv_obj_add_event_cb(btn, event_handler, LV_EVENT_CLICKED, NULL);

  lv_obj_t * cont = lv_win_get_content(win2); 
  lv_obj_t * label = lv_label_create(cont);
  lv_label_set_text(label, "This is\na pretty\nlong\ntext\nto\nsee\nhow\nthe\nwindow\n"
                      "becomes\nscrollable.\n\nSome \nmore\ntext \nto \nbe\nsure\nit\noverflows.\n   :)");
  g = lv_group_create();
  //lv_group_add_obj(g, win2);
