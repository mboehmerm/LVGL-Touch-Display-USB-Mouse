...
//#define lang_DE  // German keyboard
  ...
  const uint8_t KeyboardReportParser::numKeys[10]   PROGMEM = {'!', '"', '#', '$', '%', '&', '/', '(', ')', '='}; 
                                                            // 0x2d 0x2e                                  ? 0x32 ?                                                      0x38
  const uint8_t KeyboardReportParser::symKeysUp[12] PROGMEM = {         '?',          '`',(uint8_t)'Ü', '*', '>','\'',(uint8_t)'Ö',(uint8_t)'Ä',(uint8_t)'°', ';', ':', '_'};
  const uint8_t KeyboardReportParser::symKeysLo[12] PROGMEM = {(uint8_t)'ß', (uint8_t)'´',(uint8_t)'ü', '+', '<', '#',(uint8_t)'ö',(uint8_t)'ä',         '^', ',', '.', '-'};
  const uint8_t KeyboardReportParser::padKeys[5]    PROGMEM = {'/', '*', '-', '+', '\r'}; // en=de
#else
...
uint8_t KeyboardReportParser::OemToAscii(uint8_t mod, uint8_t key) {
  ...
  if (mod == 64) {                      // alt gr pressed ?
    if      (key == 0x14) return('@');
    else if (key == 0x08) return(uint8_t('€'));  // €  0x20ac
    else if (key == 0x10) return(uint8_t('µ'));  // µ  0xc2b5
    else if (key == 0x1f) return(uint8_t('²'));  // ²  0xc2b2
    else if (key == 0x20) return(uint8_t('³'));  // ³  0xc2b3
