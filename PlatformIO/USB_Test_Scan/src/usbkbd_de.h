...
class KeyboardReportParser {
  static const uint16_t numKeys[10];
  static const uint16_t symKeysUp[12];
  static const uint16_t symKeysLo[12];
  static const uint16_t padKeys[5];
  ...
  protected:
    ...
    virtual const uint16_t *getNumKeys() { return numKeys; };
    virtual const uint16_t *getSymKeysUp() { return symKeysUp; };
    virtual const uint16_t *getSymKeysLo() { return symKeysLo; };
    virtual const uint16_t *getPadKeys() { return padKeys; };
};
// ------------------------------------------------------------------------------------------ //
...
#ifdef lang_DE
                                                            // 0x1e 0x1f                                    0x27   
  const uint16_t KeyboardReportParser::numKeys[10]   PROGMEM = {'!', '"', '#', '$', '%', '&', '/', '(', ')', '='};           // de
                                                            // 0x2d 0x2e        ? 0x32 ?                              0x38
  const uint16_t KeyboardReportParser::symKeysUp[12] PROGMEM = {'?', '`', 'Ü', '*', '>','\'', 'Ö', 'Ä', '°', ';', ':', '_'}; // de
  const uint16_t KeyboardReportParser::symKeysLo[12] PROGMEM = {'ß', '´', 'ü', '+', '<', '#', 'ö', 'ä', '^', ',', '.', '-'}; // de
  const uint16_t KeyboardReportParser::padKeys[5]    PROGMEM = {'/', '*', '-', '+', '\r'}; // en=de
#else
  const uint16_t KeyboardReportParser::numKeys[10]   PROGMEM = {'!', '@', '#', '$', '%', '^', '&', '*', '(', ')'};           // en
  const uint16_t KeyboardReportParser::symKeysUp[12] PROGMEM = {'_', '+', '{', '}', '|', '~', ':', '"', '~', '<', '>', '?'}; // en
  const uint16_t KeyboardReportParser::symKeysLo[12] PROGMEM = {'-', '=', '[', ']', '\\',' ', ';','\'', '`', ',', '.', '/'}; // en
  const uint16_t KeyboardReportParser::padKeys[5]    PROGMEM = {'/', '*', '-', '+', '\r'}; // en=de
#endif
...
uint8_t KeyboardReportParser::OemToAscii(uint8_t mod, uint8_t key) {
  ...
  if (key == 0x64) {
    ...
    switch(key) {
      ...
      case UHS_HID_KEY_TAB        : return (0x09);
      //case UHS_HID_KEY_BACKSPACE  : return (0x1b);
    }
  }
  return ( 0);
}
